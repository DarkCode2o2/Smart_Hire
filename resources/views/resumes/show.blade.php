<x-app-layout>
    ...
    <div class="flex justify-between items-start mb-8">
        <div>
            ...
            <p class="text-xl text-blue-600 font-medium">{{ $resume->job_title }}</p>
            <p class="text-gray-500">{{ $resume->email }} | {{ $resume->experience }} Years Experience</p>
        </div>
        <a href="{{ route('dashboard') }}" class="text-lg font-semibold text-gray-600 hover:text-gray-900">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach(json_decode($resume->skills) as $skill)
                        <span class="px-3 py-1 bg-indigo-50 text-blue-700 rounded-full text-sm font-medium border border-indigo-100">
                            {{ ucfirst($skill) }}
                        </span>
                    @endforeach
                </div>
